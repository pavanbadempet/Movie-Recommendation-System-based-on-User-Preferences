    ...
    def download_artifact(self, remote_path: str, local_path: str | Path):
        ...
        if "." in remote_path.split("/")[-1] and not remote_path.endswith("/"):
            downloaded_path = hf_hub_download(
                repo_id=self.repo_id, repo_type="dataset", filename=remote_path, local_dir=str(local_path.parent)
            )
            # HF Hub downloads sometimes maintain the repo structure, ensure it maps to local_path
            shutil.move(downloaded_path, local_path)
        else:
            snapshot_download(
                repo_id=self.repo_id, repo_type="dataset", allow_patterns=f"{remote_path}/*", local_dir=str(local_path)
            )
