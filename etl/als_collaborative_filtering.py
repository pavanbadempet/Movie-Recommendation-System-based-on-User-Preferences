...
def get_spark_session() -> SparkSession:
    return (
        SparkSession.builder.appName("Nova-ALS-Collaborative-Filtering")
        .master("local[*]")
...
def extract_and_transform_events(spark: SparkSession, events_path: str) -> DataFrame | None:
    ...
    interactions = (
        raw_df.filter(col("event_type").isin("rating", "click", "view"))
    ...
    transformed = interactions.withColumn(
        ...
        .otherwise(1.0),
    ).select("user_id", col("movie_id").cast("integer"), "implicit_rating")
...
def run_als_pipeline():
    ...
    try:
        ...
        user_embeddings = user_factors.join(user_mapping, user_factors.id == user_mapping.user_idx).select(
            "user_id", col("features").alias("embedding")
        )
