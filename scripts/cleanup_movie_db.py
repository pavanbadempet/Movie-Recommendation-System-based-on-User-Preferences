...
def main():
    ...
    try:
        ...
        logger.error("If Neon project transfer limit is exceeded, this connection will fail until the limit resets or you upgrade.")
        sys.exit(1)

    # Healthcare-related tables to clean up
    healthcare_tables = [
        # Clinical & Patient tables
        "clinical_record", "fact_clinical_record", "dim_patient", "dim_doctor", "patient", "doctor",
        # Appointment tables
        "appointment", "dim_appointment", "fact_appointment",
        # Billing & Payments
        "billing", "dim_billing", "fact_billing", "payment", "transaction",
        # Care events & Operations
        "care_event", "fact_care_event", "discharge_summary", "hospital_stats", "dim_facility",
        # Terminology & Pharmacy
        "prescription", "medication", "dim_pharmacy", "medical_record", "diagnosis",
        # System & Audit
        "audit_log", "incident_report", "incident_response", "compliance_log", "alembic_version"
    ]
