...
def compute_metrics(ranks, k=10):
    ...
    return {
        "HitRate@10": hits / len(ranks),
        "NDCG@10": ndcg / len(ranks),
        "MRR": mrr / len(ranks)
    }
...
def evaluate_kan_ranker_quality():
    ...
    logger.info(f"KAN Ranker Metrics: HR@10={metrics['HitRate@10']:.3f}, NDCG@10={metrics['NDCG@10']:.3f}, MRR={metrics['MRR']:.3f}")
    return metrics
...
def evaluate_hyperbolic_quality():
    ...
    metrics = compute_metrics(ranks, k=10)
    logger.info(f"Hyperbolic Metrics: HR@10={metrics['HitRate@10']:.3f}, NDCG@10={metrics['NDCG@10']:.3f}, MRR={metrics['MRR']:.3f}")
    return metrics
