...
class MultiTaskDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if torch.rand(1).item() < 0.5:
            return (
                self.users[idx], self.items[idx], self.positions[idx],
                self.click[idx], self.watch[idx], self.sat[idx]
            )
        else:
            # Negative sample
            neg_item = torch.randint(0, self.num_items, (1,)).item()
            return (
                self.users[idx], torch.tensor(neg_item, dtype=torch.long), self.positions[idx],
...
def main():
    ...
    if user_emb_path.exists() and item_emb_path.exists():
        try:
            ...
            if len(u_mat) >= num_users: u_mat = u_mat[:num_users]
            else: u_mat = np.pad(u_mat, ((0, num_users - len(u_mat)), (0, 0)))
            
            if len(i_mat) >= num_items: i_mat = i_mat[:num_items]
            else: i_mat = np.pad(i_mat, ((0, num_items - len(i_mat)), (0, 0)))
    ...
    for epoch in range(1, epochs + 1):
        ...
        logger.info(f"Epoch {epoch:2d} | Train Loss: {avg_loss:.4f} | Val Loss: {val_loss:.4f} | CTR: {total_ctr/len(train_loader):.4f} | Watch: {total_watch/len(train_loader):.4f} | Sat: {total_sat/len(train_loader):.4f}")
