...
if not ML25_DIR.exists():
    ...
    urllib.request.urlretrieve("https://files.grouplens.org/datasets/movielens/ml-25m.zip", DATA_DIR / "ml-25m.zip")
...
torch.save(
    {
        "user_emb": model.user_emb.weight.data.cpu(),
        ...
        self.blocks = nn.ModuleList(
            [
                nn.TransformerEncoderLayer(
                    d_model=d, nhead=n_heads, dim_feedforward=d * 4, dropout=dropout, batch_first=True, norm_first=True
                )
...
for epoch in range(SAS_EPOCHS):
    ...
    for s in range(0, len(perm), SAS_BATCH):
        ...
        out = sas_model(st)[:, -1, :]
        pe = sas_model.item_emb(pt)
        ne = sas_model.item_emb(nt)
