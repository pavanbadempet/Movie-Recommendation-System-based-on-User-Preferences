...
class ApexEnsembleEngine(nn.Module):
    ...
    def __init__(self, num_users: int = 1000, num_items: int = 10000, emb_dim: int = 16, device: str | None = None):
        ...
        self._session_cache: dict[str, tuple[float, list[int]]] = {}
    ...
    def _inject_pyspark_priors(self):
        ...
        if user_emb_path.exists() and item_emb_path.exists():
            try:
                ...
                logger.info(f"✅ Successfully injected Real PySpark Embeddings ({self.num_users} users, {self.num_items} items) into all 4 models.")
    ...
    def _load_trained_weights(self):
        ...
        models_to_load = {
            ...
            "lightgcn": MODELS_DIR / "lightgcn.pth",
        }
    ...
    def _get_session_sequence(
        ...
        try:
            ...
            try:
                ...
                pass  # Fall through to background index
            ...
            # Cache the result — evict oldest if over 10000 entries
            if len(self._session_cache) >= 10000:
                try:
                    self._session_cache.pop(next(iter(self._session_cache)))
                except StopIteration:
    ...
    def predict_ensemble(self, user_id: int, candidate_item_ids: list[int], session_sequence: list[int] | None = None) -> dict[int, float]:
        """
    ...
    def _predict_ensemble_onnx(self, user_id: int, candidate_item_ids: list[int], onnx, session_sequence: list[int] | None = None) -> dict[int, float]:
        ...
        try:
            with torch.no_grad():
                u_emb = self.lightgcn.user_embedding(torch.tensor([safe_user_id])).expand(len(safe_item_ids), -1).numpy()
        ...
        try:
            with torch.no_grad():
                u_emb_d = self.lightgcn.user_embedding(torch.tensor([safe_user_id])).expand(len(safe_item_ids), -1).numpy()
        ...
        # Blend
        key_map = {"lightgcn": "lightgcn", "quantum": "quantum", "sasrec": "sasrec",
                   "kan": "kan", "hyperbolic": "hyperbolic", "diffusion": "diffusion"}
    ...
    def _predict_ensemble_pytorch(self, user_id: int, candidate_item_ids: list[int], session_sequence: list[int] | None = None) -> dict[int, float]:
        if not candidate_item_ids:
            return {}
        ...
        try:
            ...
            pass  # Fall back to base user embedding
        ...
        try:
            ...
            # Run all 6 models in parallel threads
