        ...
        if request.url.path.startswith("/v1/recommendations") or request.url.path.startswith("/v1/search/ai"):
            try:
                ...
                if requests_this_second > limit:
                    logger.warning(f"s [RATE LIMIT EXCEEDED] Tenant {tenant_id} (Tier: {tenant_tier}) exceeded {limit} TPS.")
                    return JSONResponse(
                        status_code=429,
                        content={"error": "SLA Quota Exceeded. Please upgrade to a higher Enterprise Tier for more TPS."},
