...
from backend.pipeline.pipeline_types import CandidateItem, RankedItem
...
class RankingConfig:
    ...
    use_learned_ranker:
        ...
        ``True``.
    apply_ips_reranking:
        When ``True``, apply Inverse Propensity Scoring re-weighting to
        correct for popularity bias in the final ranking.  Popular items
        are down-weighted proportionally to their exposure frequency so that
        niche items the user genuinely prefers surface above blockbusters.
        Defaults to ``True``.
    item_popularity:
        Optional pre-computed popularity map (``movie_id → float``) for IPS
        re-ranking.  When ``None`` and ``apply_ips_reranking=True``, the
        pipeline attempts to load popularity from the event store on first
        call (cached across requests).  Defaults to ``None``.
    ips_clip_val:
        Maximum IPS weight multiplier to prevent extreme re-weighting on
        very rare items.  Defaults to ``5.0``.
    """
...
class RankingPipeline:
    ...
    def rank(
        ...
        # ----------------------------------------------------------------
        # Step 3b: IPS popularity-bias correction (optional)
        # Apply 1/popularity re-weighting so niche items the model scores
        # well aren't buried beneath blockbusters with equal model scores.
        # ----------------------------------------------------------------
        if self.config.apply_ips_reranking:
            popularity = self._get_popularity()
            if popularity:
                mean_pop = sum(popularity.values()) / max(len(popularity), 1)
                clip = max(self.config.ips_clip_val, 1.0)
                ips_blended: list[tuple[CandidateItem, float, float, float]] = []
                for candidate, ens_score, rnk_score, final_score in blended:
                    pop = popularity.get(candidate.movie_id, mean_pop)
                    ips_weight = min(1.0 / max(pop, 1e-6), clip)
                    # Soft application: blend 20% IPS correction into score
                    # so popular items are gently penalised, not excluded
                    adjusted_score = final_score * (0.8 + 0.2 * ips_weight / clip)
                    ips_blended.append((candidate, ens_score, rnk_score, adjusted_score))
                blended = ips_blended
                logger.debug("IPS re-ranking applied to %d candidates.", len(blended))
    ...
    def _get_popularity(self) -> "dict[int, float]":
        ...
        """
        # 1. Explicit config value takes precedence (e.g. pre-computed nightly)
        if self.config.item_popularity is not None:
            return self.config.item_popularity

        # 2. Cached from a previous request
        ...
        try:
            ...
            if events:
                self._popularity_cache = compute_item_popularity(events)
                logger.debug("IPS popularity loaded: %d items", len(self._popularity_cache))
            else:
                self._popularity_cache = {}
        except Exception as exc:
            logger.debug("IPS popularity load failed; skipping re-ranking: %s", exc)
            self._popularity_cache = {}
