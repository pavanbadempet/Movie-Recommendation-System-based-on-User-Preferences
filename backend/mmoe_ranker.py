...
class MMoERanker(nn.Module):
    def __init__(
        ...
        # 1. Experts
        self.experts = nn.ModuleList([Expert(input_dim, expert_hidden_dim, expert_out_dim) for _ in range(num_experts)])

        # 2. Gates (One per task)
        # Task 1: CTR (Click-Through Rate)
        # Task 2: Watch Time
        # Task 3: Satisfaction (Rating >= 4.0)
        self.gate_ctr = nn.Sequential(nn.Linear(input_dim, num_experts), nn.Softmax(dim=1))

        self.gate_watch = nn.Sequential(nn.Linear(input_dim, num_experts), nn.Softmax(dim=1))

        self.gate_sat = nn.Sequential(nn.Linear(input_dim, num_experts), nn.Softmax(dim=1))
