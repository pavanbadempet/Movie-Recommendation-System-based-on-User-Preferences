...
class HardwareProfile:
    ...
    ram_gb: float
    cpu_cores: int
    gpu_vram_gb: float = 0.0  # 0.0 when no GPU or VRAM query fails
...
class TierDetector:
    ...
    def detect(self) -> HardwareProfile:
        ...
        # GPU VRAM — only meaningful when a GPU is present.
        # Tier1 requires ≥ 8 GB VRAM to fit the full 6-model ensemble on-device.
        ...
        if gpu_available:
            try:
                ...
                gpu_vram_gb = vram_bytes / (1024**3)
            except Exception as exc:
                logger.warning(
                    "GPU VRAM detection failed (%s); Tier1 auto-detection will require "
                    "explicit NOVA_SERVING_TIER override to activate.",
                    exc,
                )
                gpu_vram_gb = 0.0
        ...
        self._profile = HardwareProfile(
            ...
            gpu_vram_gb=float(max(gpu_vram_gb, 0.0)),
        )
        self._detected = True
        logger.info(
            "Hardware detected: gpu=%s, gpu_vram_gb=%.1f, ram_gb=%.1f, cpu_cores=%d",
            self._profile.gpu_available,
            self._profile.gpu_vram_gb,
    ...
    def resolve(self) -> tuple[str, str]:
        ...
        self._reason = reason
        logger.info(
            "Serving tier resolved: %s (reason=%s, gpu=%s, gpu_vram_gb=%.1f, ram_gb=%.1f, cpu_cores=%d)",
            tier,
            reason,
    ...
    # Minimum GPU VRAM required for Tier1 (full 6-model ensemble + FAISS on-device).
    # Empirically measured: ~7 GB at emb_dim=16 / 10k items.  8 GB provides headroom.
    _TIER1_MIN_VRAM_GB: float = 8.0
    ...
    def _auto_select(self, profile: HardwareProfile) -> tuple[str, str]:
        ...
        if profile.gpu_available and profile.ram_gb >= 16.0:
            # Require sufficient VRAM as well.  If VRAM query failed (gpu_vram_gb == 0.0),
            # fall back to Tier2 to avoid OOM — an explicit override is needed to activate Tier1.
            if profile.gpu_vram_gb >= self._TIER1_MIN_VRAM_GB:
                return "tier1", "hardware_auto_detection"
            elif profile.gpu_vram_gb == 0.0:
                logger.warning(
                    "GPU detected but VRAM could not be measured; defaulting to tier2. "
                    "Set NOVA_SERVING_TIER=tier1 to override."
                )
            else:
                logger.warning(
