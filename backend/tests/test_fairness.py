...
def test_gini_popularity_bias():
    ...
    gini_extreme = auditor.measure_popularity_bias(extreme_slates)
    # Gini of a single item taking 100% of the distribution is mathematically 0.0 in this specific array size, 
    # wait, actually a completely unequal distribution where 1 person has everything approaches 1.0.    # To get a Gini > 0.5, we need a long tail of items with 1 recommendation, and one item with many
