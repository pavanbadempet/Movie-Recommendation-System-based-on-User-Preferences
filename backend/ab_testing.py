...
class MultiArmedBandit:
    ...
    def __init__(
        ...
        # Policy metrics
        self.metrics: dict[str, ExperimentMetrics] = {policy.name: ExperimentMetrics() for policy in policies}
    ...
    def select_policy(self, user_id: str | None = None) -> RecommendationPolicy:
        """
    ...
    def _epsilon_greedy_select(self) -> RecommendationPolicy:
        ...
        if np.random.random() < self.epsilon:
            ...
            best_policy_name = max(self.metrics.keys(), key=lambda k: self.metrics[k].avg_reward)
    ...
    def update_policy(
        self, policy_name: str, clicked: bool = False, converted: bool = False, reward: float = 0.0
    ) -> None:
        """
...
class ABTestManager:
    ...
    def assign_policy(self, user_id: str) -> RecommendationPolicy:
        ...
        hash_value = int(hashlib.sha256(f"{self.test_name}:{user_id}".encode()).hexdigest(), 16)
    ...
    def calculate_significance(self, metric: str = "ctr", alpha: float = 0.05) -> dict[str, Any]:
        """
