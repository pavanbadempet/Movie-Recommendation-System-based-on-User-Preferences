            ...
            try:
                ...
                if not hasattr(self, '_prometheus_gauge_state'):
                    self._prometheus_gauge_state = self.prometheus_client.Gauge(
                        ...
                        f'Circuit breaker state for {self.name}'
                    )
                    self._prometheus_gauge_failure_rate = self.prometheus_client.Gauge(
                        f'circuit_breaker_{self.name}_failure_rate',
                        f'Failure rate for {self.name}'
                    )
                    self._prometheus_counter_total = self.prometheus_client.Counter(
                        f'circuit_breaker_{self.name}_total_calls',
                        f'Total calls for {self.name}'
                    )
                    self._prometheus_counter_success = self.prometheus_client.Counter(
                        f'circuit_breaker_{self.name}_successful_calls',
                        f'Successful calls for {self.name}'
                    )
                    self._prometheus_counter_failed = self.prometheus_client.Counter(
                        f'circuit_breaker_{self.name}_failed_calls',
                        f'Failed calls for {self.name}'
                    )
                    self._prometheus_counter_rejected = self.prometheus_client.Counter(
                        f'circuit_breaker_{self.name}_rejected_calls',
                        f'Rejected calls for {self.name}'
                    )
...
def create_tmdb_circuit_breaker(prometheus_client: Optional[Any] = None) -> CircuitBreaker:
...
def create_openrouter_circuit_breaker(prometheus_client: Optional[Any] = None) -> CircuitBreaker:
...
def create_redis_circuit_breaker(prometheus_client: Optional[Any] = None) -> CircuitBreaker:
