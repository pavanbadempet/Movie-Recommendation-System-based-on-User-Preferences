...
class ContentUnderstandingEngine:
    ...
    def extract_themes_and_moods(self, overview: str, top_k: int = 2) -> dict[str, list[str]]:
        ...
        top_themes = [
            label for label, score in zip(theme_results["labels"], theme_results["scores"], strict=False) if score > 0.5
        ][:top_k]

        # 2. Extract Moods
        mood_results = self.classifier(overview, self.moods, multi_label=True)
        top_moods = [
            label for label, score in zip(mood_results["labels"], mood_results["scores"], strict=False) if score > 0.5
        ][:top_k]

        return {"themes": top_themes, "moods": top_moods}
