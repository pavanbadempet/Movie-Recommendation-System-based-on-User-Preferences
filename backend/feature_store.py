...
class FeatureStore:
    def __init__(self, data_dir: Path | str = None):
        self.data_dir = Path(data_dir) if data_dir else Path(__file__).resolve().parent.parent / "data" / "als_embeddings"
    ...
    def load(self):
        ...
        if not user_path.exists() or not item_path.exists():
            logger.warning(f"Feature Store artifacts not found at {self.data_dir}. Collaborative Filtering is disabled.")
            return False
